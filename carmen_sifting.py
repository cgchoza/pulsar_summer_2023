...
def reject_highfrequency(candlist, *args, **kwargs):
    """Remove candidates with DM problems. The candlist is modified
        **in-place**.
        ...
        Output:
            candlist: The modified candidate list.
    """
    candlist.reject_highfrequency(*args, **kwargs)
    return candlist
...
def remove_many_DMs(candlist, *args, **kwargs):
    """Remove candidates with DM problems. The candlist is modified
        **in-place**.
        ...
        Output:
            candlist: The modified candidate list.
    """
    candlist.remove_many_DMs(*args, **kwargs)
    return candlist

def remove_low_DM_variance(candlist, *args, **kwargs):

    candlist.remove_low_DM_variance(*args, **kwargs)
    return candlist

def remove_known_harmonics(candlist, *args, **kwargs):
    """Remove harmonics. The candlist is modified
        **in-place**.

        Note: This function is defined to maintain support
            for old code. It simply calls the 
            'remove_known_harmonics' method of candlist.
        ...
        Output:
            candlist: The modified candidate list.
    """
    candlist.remove_known_harmonics(*args, **kwargs)
    return candlist
...
class Candlist(object):
    ...
    def mark_as_bad(self, icand, badlistname, error_file=None):
        cand = self.cands.pop(icand)
        if self.trackbad:
            badlist = self.badlists.setdefault(badlistname, [])
            badlist.append(cand)
        if self.trackdupes:
            key = f"{cand.filename}:{cand.candnum}"
            if key in self.duplicates.keys():
                self.duplicates.pop(key)

    def mark_as_duplicate(self, icand, bestcand):
        cand = self.cands.pop(icand)
        if self.trackdupes:
            key = f"{bestcand.filename}:{bestcand.candnum}"
            self.duplicates.setdefault(key, []).append(cand)
    ...
    def get_all_goodcands(self):
        duplicates = [dupe for dupelist in self.duplicates.values() for dupe in (dupelist if isinstance(dupelist, list) else [dupelist])]
        return self.cands + duplicates
    ...
    def reject_highfrequency(self, error_file, high_frequency=None):
        """Find and remove candidates outside of the appropriate range for millisecond pulsars.
            High frequency candidates are moved to the 'highfrequency' bad-list.

            Inputs:
                high_frequency: The highest allowed frequency for a "good" cand.
        ...
        for ii in reversed(list(range(len(self.cands)))):
            cand = self.cands[ii]
            if (cand.f > high_frequency):
                cand.note = "Frequency is too high (%g ms < %g ms)" % \
                            (cand.f, high_frequency)
                self.mark_as_bad(ii, 'highfrequency', error_file)

    def reject_knownbirds(self, error_file, known_birds_f=[], known_birds_p=[]):
    ...
    def default_rejection(self, error_file, known_birds_f):
        """Run all rejection methods with default arguments.
        ...
        self.reject_knownbirds(error_file, known_birds_f=known_birds_f)
        self.reject_threshold()
        self.reject_harmpowcutoff()
        self.reject_rogueharmpow()

    def remove_duplicate_candidates(self, error_file, verbosity=1):
    ...
    def remove_known_harmonics(self, error_file, verbosity=1):
        """Remove the candidates that are lower signifiance harmonics of zapped known pulsars.
        
            Inputs:
                verbosity: Verbosity level. (Default: 1)
                
            Outputs:
                None. Marks known harmonics as bad."""
        numremoved = 0
        self.cands.sort(key=lambda cand: cand.sigma, reverse=True)
        f_err = r_err / self.cands[0].T
        if verbosity >= 1:
            print("\nSearching for known pulsar harmonics...")
        ii = 0
        while True:
            pulsar_freq = known_birds_f[ii][0]
            jj = len(self.cands) - 1
            zapj = 0
            while True:
                harmcand = self.cands[jj]
                for factor in np.arange(1.0, 17.0):
                    if np.fabs(pulsar_freq - harmcand.f*factor) < f_err*factor:
                        zapj = 1
                        harmstr = "1/%dth" % factor
                    elif np.fabs(pulsar_freq - harmcand.f/factor) < f_err/factor:
                        zapj = 1
                        if factor==2.0:
                            harmstr = "%dnd" % factor
                        else:
                ...
                if zapj:
                    numremoved += 1
                    self.mark_as_bad(jj, 'harmonic', error_file)
                    # for id in reversed(list(range(len(self.duplicates)))):
                    #     note = self.duplicates[id].note
                    #     if 'duplicate' in note and harmcand.filename in note and str(harmcand.candnum) in note:
                    #         self.duplicates.pop(id)
                    zapj = 0
                jj -= 1
                if jj == ii:
                    break
            ii += 1
            if ii >= len(known_birds_f) - 1:
                break


    def remove_harmonics(self, error_file, verbosity=1):
        """Remove the candidates that are lower significance harmonics
            ...
            Ouputs:
                None
        """
        # Note:  should probably put the harmonics into the fundcand.f as hits (use sets)
        numremoved = 0
        # self.cands.sort(key=attrgetter('sigma'), reverse=True)
        self.cands.sort(key=lambda cand: cand.sigma, reverse=True)
        f_err = r_err / self.cands[0].T
    ...
    def remove_many_DMs(self, error_file, verbosity=1):
        """For each candidate, calculate a fit of the hits to a straight line across DMs, 
        calculate the chi-squared statistic, and remove the candidates with low deviance 
        from a straight line model. 
            ...
            Outputs:
                None. Marks candidates with no peak as bad."""
        for ii in reversed(list(range(len(self.cands)))):
                currcand = self.cands[ii]

                fit_DMs = np.array([c[0] for c in currcand.hits])     # Get a list of DMs for all the hits assigned to this cand
                
                DM_standard_deviation = np.std(fit_DMs)

                standard_deviation_threshold = 2 # Start conservative
                if DM_standard_deviation > standard_deviation_threshold:
                    self.mark_as_bad(ii, 'dmproblem', error_file)
                    # Remove corresponding duplicates
                    # currcand_file = currcand.filename
                    # currcand_candnum = currcand.candnum
                    # for id in reversed(list(range(len(self.duplicates)))):
                    #     note = self.duplicates[id].note
                    #     if 'duplicate' in note and currcand_file in note and str(currcand_candnum) in note:
                    #         if verbosity >= 2:
                    #             error_file.write(f"Duplicate note is {self.duplicates[id].note}")
                    #         self.duplicates.pop(id)
                    continue

    def remove_low_DM_variance(self, error_file, verbosity=1):
        """For each candidate, calculate a fit of the hits to a straight line across DMs, 
        calculate the chi-squared statistic, and remove the candidates with low deviance 
        from a straight line model. 
        ...
        for ii in reversed(list(range(len(self.cands)))):
                ...
                if len(currcand.hits) < 10:
                    continue

                fit_DMs = np.array(currcand.hits[:][0])     # Get a list of DMs for all the hits assigned to this cand
                fit_sigmas = np.array(currcand.hits[:][2])  # Get a list of sigmas for all the hits assigned to this cand
                uncertainties = np.array([1]*len(fit_sigmas))  # NOTE: get a more reasonable number for this--coherent power better?

                params, covariance = curve_fit(linear_fit, fit_DMs, fit_sigmas)
                m_fit, b_fit = params

                y_fit = linear_fit(fit_DMs, m_fit, b_fit)
                residuals = fit_sigmas - y_fit
                chi_squared = np.sum((residuals / uncertainties)**2)

                chi_squared_threshold = 5 # Start conservative
                if chi_squared < chi_squared_threshold:
                    self.mark_as_bad(ii, 'dmproblem', error_file)

                    # Remove corresponding duplicates
                    # Put them into a dictionary, pop entire keys, relist duplicates?
                    # currcand_file = currcand.filename
                    # currcand_candnum = currcand.candnum
                    # for id in reversed(list(range(len(self.duplicates)))):
                    #     note = self.duplicates[id].note
    ...
    # def remove_bad_duplicates(self):
    #     for ii in reversed(list(range(len(self.duplicates)))):
    #             currcand = self.duplicates[ii]
    #             currcand_file = currcand.filename
    #             currcand_candnum = currcand.candnum
    #                 if 'duplicate' in note and currcand_file in note and str(currcand_candnum) in note:
    #                     if verbosity >= 1:
    ...
    def remove_DM_problems(self, numdms, dmlist, low_DM_cutoff, error_file, verbosity=1):
...
def candlist_from_candfile(filename, error_file, trackbad=False, trackdupes=False):
    ...
    for line in candfile:
        ...
        if fund_re.match(line):
            ...
            DMstr = filename.split("DM")[-1].split("_")[0]
